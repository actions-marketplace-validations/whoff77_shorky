...
exports.SHORKY_AGENT_TOOLS = void 0;
...
function pushTrace(traceLogs, entry) {
    if (!traceLogs)
        return;
 ...
 */
async function inspectInteractiveElements(page) {
 ...
 */
async function resolveWithFallback(page, originalSelector, intent, traceLogs) {
    ...
    try {
        ...
        const byRole = page.getByRole(roleGuess, { name: originalSelector, exact: false });
    ...
    try {
        const healedSelector = await (0, healingEngine_1.healSelector)(page, originalSelector);
 ...
 */
async function executeAgentTool(page, toolName, args, autoHealPage, traceLogs) {
