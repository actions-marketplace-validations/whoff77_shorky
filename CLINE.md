...
- **`src/fixtures/autoHealFixture.ts`** — a custom Playwright fixture (`test.extend`) providing `autoHealPage` with `clickAndHeal()`, `assertVisual()`, `assertVisualBaseline()`. `clickAndHeal()` is a strict pass-through (`page.click(selector)`) — it intentionally does **not** cache or fall back to runtime self-healing; a failing selector must throw a normal Playwright timeout error so `fixTrace.ts` can permanently repair the `.spec.ts` source file itself (the finalized architecture — see the GitHub Action description above). Also resets the worker-local token counter (`resetTokensUsedThisTest()`) at fixture setup and, at teardown, attaches the tokens consumed during the test (`SHORKY_TOKENS_ATTACHMENT_NAME`) via `testInfo.attach()` for `cloudReporter.ts` to pick up — see "LLM Token Usage Tracking" below.
- **`src/reporters/cloudReporter.ts`** — custom Playwright reporter, only registered in `playwright.config.ts` when `isShorkyCloudEnabled()` is true; POSTs run/test telemetry to `shorky-cloud`, including per-test and run-level `tokensUsed` totals (read back out of each test's `SHORKY_TOKENS_ATTACHMENT_NAME` attachment in `onTestEnd()`).
- **`src/config/shorkyCloud.ts`** — single source of truth for all shorky-cloud URLs/env resolution (`getShorkyCloudTelemetryUrl()`, `getShorkyCloudWebhookUrl()`, `getShorkyCloudPreflightUrl()`); always extend here rather than hardcoding URLs elsewhere.
...
5. **Webhook path (`src/cli/fixTrace.ts`'s `notifyShorkyCloud()`)** — since `autoHealFixture.ts`'s `clickAndHeal()` is now a strict pass-through (no runtime self-healing, no token usage there), the tokens tracked for the GitHub Action's fix flow come entirely from `codeFixer.ts`'s `generateSpecFix()` (`response.usage.total_tokens`, returned as `FixResult.tokensUsed`). `runOfflineFix()` threads that value into `HealedFixEntry.tokensUsed`, which both the standalone `notifyShorkyCloud()` dispatch and the batch `notifyShorkyCloudBatch()` loop forward as `tokensUsed` on the `/api/webhook` payload — matching `shorky-cloud`'s `webhookPayloadSchema`'s optional `tokensUsed` field, which atomically increments `projects.tokensUsedThisMonth` the same way `/api/v1/telemetry` does.
