...
- **`tests/`** — Playwright specs: generated deterministic specs (`generated-login.spec.ts`), a visual-regression baseline demo (`baseline.spec.ts`), and an intentionally-broken spec (`broken-login.spec.ts`) used to exercise the "fail-and-rewrite" self-healing flow end-to-end. The legacy runtime-injection demo spec (`self-healing.spec.ts`, which exercised the now-removed `clickAndHeal()` fixture method) has been deleted as dead MVP code. Snapshots live under `tests/__snapshots__/` and `tests/generated-login.spec.ts-snapshots/` per `playwright.config.ts`'s `snapshotPathTemplate`.
