...
- `SHORKY_CLOUD_URL` / `SHORKY_CLOUD_API_KEY` — optional; enables the Playwright reporter + webhook + pre-flight governance check integration with `shorky-cloud` (`isShorkyCloudEnabled()` in `src/config/shorkyCloud.ts`). When unset, the pre-flight check (`src/cli/preflight.ts`) is skipped entirely and the LLM repair loop always proceeds (no budget/quota to enforce for unconfigured/local runs).
